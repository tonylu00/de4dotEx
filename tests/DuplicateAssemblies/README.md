...
`Invoke-ExternalContextRegression.ps1` runs the batch suite and then tests `--assembly-contexts` with a dependency outside the input tree, found through the context's configuration probing path. The host client must follow renamed host definitions, while the legacy client stays byte-identical and runs against the external implementation. Supplying a same-identity external library without the required API must fail publication.
