...
$lib=Join-Path $OutputDirectory 'lib'; $caller=Join-Path $OutputDirectory 'caller'
New-Item -ItemType Directory $lib,$caller | Out-Null
...
'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>net48</TargetFramework><OutputType>Exe</OutputType></PropertyGroup><ItemGroup><ProjectReference Include="..\lib\Fixture.csproj"/></ItemGroup></Project>' | Set-Content "$caller\Caller.csproj"
