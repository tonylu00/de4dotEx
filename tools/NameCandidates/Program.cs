...
public static class Scanner {
    ...
    static string Fingerprint(MethodDef m) {
        ...
        string Operand(object operand) => operand switch {
            ...
            IMethod r => Json(new object[] { "method", r.Name.String, Scope(r.DeclaringType), Signature(r.MethodSig),
            ...
            try {
                ...
                ILookup<string, MethodDef> Index(IEnumerable<MethodDef> methods) {
                    ...
                    foreach (var method in methods) {
                        try { entries.Add((Fingerprint(method), method)); }
                        catch (NotSupportedException ex) { report.Skips.Add(new(relative, ex.Message, method.MDToken.ToString())); }
                    }
                ...
                foreach (var group in next) {
                    ...
                    if (matches.Length != 1 || group.Count() != 1) { report.Skips.Add(new(relative, "Ambiguous fingerprint " + group.Key)); continue; }
                    var method = group.Single(); var reference = matches[0];
                    ...
                    report.Candidates.Add(new(relative, a.Assembly.FullName, b.Assembly.FullName, a.Mvid.ToString(), b.Mvid.ToString(), aHash, bHash,
                        reference.MDToken.ToString(), method.MDToken.ToString(), reference.FullName, method.FullName, reference.Name.String, group.Key,
                        Visible(method.DeclaringType) && (method.IsPublic || method.IsFamily || method.IsFamilyOrAssembly)));
                }
            }
