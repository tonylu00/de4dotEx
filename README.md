...
After protector cleanup, batch mode also repairs malformed attribute type values that name a missing local core-library `System` type (for example, `System.String, MyAssembly`) by restoring the core-library scope. Non-primitive types such as `System.DateTime` must resolve in the module's core library before repair. Actual local definitions and exported types are preserved; this is not a general fallback for unresolved types. Unprotected components with this repair are saved and reported as reference-only changes.
